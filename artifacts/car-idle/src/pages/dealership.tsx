...
function CarCard({ car }: { car: Car }) {
  ...
  return (
    ...
    `}>
      {count > 0 && (
        <div className="absolute top-3 right-3 bg-primary text-primary-foreground text-xs font-black px-2 py-0.5 rounded-full z-10">
          x{count}
        </div>
      )}
      ...
      <div className="w-full h-20 flex items-center justify-center bg-background/50 rounded-lg overflow-hidden">
        <svg viewBox="0 0 120 60" className="w-28 drop-shadow-lg" fill="none">
      ...
      {/* Info */}
      <div>
        <div className="font-black text-white text-sm leading-tight">{car.name}</div>
        <div className="text-xs text-muted-foreground">{car.brand}</div>
        <Badge variant="outline" className={`mt-1 text-[10px] rarity-${car.rarity} border-current/30 px-1.5 py-0`}>
          {car.rarity}
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-2 gap-1.5 text-xs">
        <div className="bg-background/60 rounded p-1.5">
          <div className="text-muted-foreground uppercase tracking-wider text-[9px]">Passive</div>
          <div className="font-bold text-accent text-xs">{formatNumber(car.milesPerSecond)}/s</div>
        </div>
        <div className="bg-background/60 rounded p-1.5">
          <div className="text-muted-foreground uppercase tracking-wider text-[9px]">Click</div>
          <div className="font-bold text-primary text-xs">x{car.clickMultiplier}</div>
        </div>
      </div>
      ...
      <Button
        ...
        size="sm"
        className={`
          w-full font-bold uppercase tracking-wider text-xs py-2
          ${canAfford && !isLocked
...
export default function Dealership() {
  ...
  return (
    <div className="flex flex-col gap-5 animated-bg">
      <div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-card border border-border/50 p-1 flex-wrap h-auto gap-1">
          {categories.map(cat => (
            <TabsTrigger
              key={cat.id}
              value={cat.id}
              className={`font-bold uppercase tracking-wider text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground ${cat.id === "hypercar" ? "data-[state=active]:bg-amber-500" : ""}`}
            >
              {cat.label}
            </TabsTrigger>
          ))}
        ...
        {categories.map(cat => (
          <TabsContent key={cat.id} value={cat.id} className="mt-4">
            {cat.id === "hypercar" && (
              <div className="mb-4 p-3 rounded-xl border border-amber-500/30 bg-amber-500/5 text-xs text-amber-400 font-bold uppercase tracking-wider">
                ⚡ Hypercars require Prestige to unlock. These are the fastest machines ever built.
              </div>
            )}
