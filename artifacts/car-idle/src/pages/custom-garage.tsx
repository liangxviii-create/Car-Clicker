...
function CustomVehicleCard({ vehicle, onDelete }: { vehicle: CustomVehicle; onDelete: () => void }) {
  ...
  // miles/s derived from HP (1 HP ≈ 10 miles/s baseline)
    ...
    <div className="flex flex-col gap-6 animated-bg">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Design unlimited custom vehicles. Upload images, set specs, and add them to your collection.
        </p>
      </div>
      ...
      {showForm && (
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <label className="flex flex-col gap-1">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Horsepower</span>
              <input
                type="number"
                min={1}
                value={form.horsepower}
                onChange={e => setForm(p => ({ ...p, horsepower: Number(e.target.value) || 0 }))}
      ...
      <div className="bg-card border border-border/50 rounded-xl p-4 text-xs text-muted-foreground">
        ...
        <p>Your custom vehicles earn miles based on their horsepower (1 HP = 10 miles/sec passively). Images you upload are stored locally in your browser save. Use the Dyno to test your creations!</p>
      </div>
    </div>
  );
}
